...
public final class CanvasRenderer {
	...
	public static void addShutdownHook() {
		if (CanvasRenderer.shutdownHook == null) {
			...
			Runtime.getRuntime().addShutdownHook(new Thread(CanvasRenderer.shutdownHook));
		}
	}
	...
	private static final class GraphicsPanel extends JPanel {
		...
		public GraphicsPanel(int rows, int columns) {
			...
			Dimension size = new Dimension(this.columns * pixelSize + 2 * borderSize,
		...
		public void setImage(int[][] image) {
			...
			this.image = new BufferedImage(this.columns * pixelSize, this.rows * pixelSize, BufferedImage.TYPE_INT_RGB);
			for (int c = 0; c < columns; c++) {
				for (int r = 0; r < rows; r++) {
					for (int cc = 0; cc < pixelSize; cc++) {
						for (int rr = 0; rr < pixelSize; rr++) {
							int color = image[r][c];
							if (showGridLines && (rr == 0 || rr == pixelSize - 1 || cc == 0 || cc == pixelSize - 1)) {
								color = 0;
							}
							this.image.setRGB(c * pixelSize + cc, r * pixelSize + rr, color);
						}
					}
				}
			}
